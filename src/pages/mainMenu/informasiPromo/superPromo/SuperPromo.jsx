...
function SuperPromo() {
  ...
  const [dtSuperPromo, setDtSuperPromo] = useState(null);
  ...
  useEffect(() => {
    ...
    const getLoadSuperPromo = async () => {
      setLoading(true);
      await axios
        .get(`${URL_GATEWAY}/serviceInfoPromo/getInfoSuperPromo`, {
          headers: {
        ...
        .then((response) => {
          if (response["data"]) {
            ...
            // for (let i = 0; i < response["data"]["dataPic"][0].length; i++) {
            //   dtPic.push(response["data"]["dataPic"][0][i]["GPR_PICTURE"]);
            // }
            ...
            console.log("response", response["data"]["picArr"]);
            setPictureArr(response["data"]["picArr"]);
            setDtSuperPromo(response["data"]["infoSuperPromo"]);
            console.log(dtSuperPromo);
            setLoading(false);
          } else {
            setDtSuperPromo(false);
            setLoading(false);
          }
          setLoading(false);
          console.log("dtSuperPromo: ", dtSuperPromo);
        })
      ...
      >
        ...
        {dtSuperPromo ? (
                  ...
                  <SwiperSlide className="max-w-[426.66px]">
                    ...
                    >
                      <p className="text-[24px] font-semibold text-center">
                        {dtSuperPromo[itemIndex]["sph_keterangan"]}
                      </p>
                      <p className="text-[20px] font-semibold text-center">
                        Rp {dtSuperPromo[itemIndex]["sph_harga"].trim()}
                      </p>
                    </div>
          ...
          <DataEmpty
            ...
            title="Tidak Ada Super Promo Yang Sedang Berlaku"
          />
        )}

        <div
          className={`flex flex-row justify-between mt-auto ${
            dtSuperPromo ? null : "self-end"
          }`}
        >
          {dtSuperPromo ? (
            <div
            ...
            >
              <p className="font-semibold text-center text-white text-subText">
                Periode: {dtSuperPromo ? dtSuperPromo[0]["tglberlaku"] : "-"}
              </p>
            </div>
          ) : null}
